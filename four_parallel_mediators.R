...
calc_4mediator_power <- function(powReps = 10, mcmcReps = 10, seed = 2, 
  ...
  df <- data.frame("Parameter" = c("a1b1", "a2b2", "a3b3","a4b4",
                                   "difference12", "difference13","difference14",
                                   "difference23", "difference24", "difference34"),
                   "N" = rep(N, 10),
...
if (input_method == "corr") {
  ...
  input_data <- data.frame(a1=runif(numIterations,min= cor21_low,max = cor21_high),
                           ...
                           a4=runif(numIterations,min= cor51_low,max = cor51_high),
                           b1=runif(numIterations,min= cor62_low,max = cor62_high),
                           b2=runif(numIterations,min= cor63_low,max = cor63_high),
                           b3=runif(numIterations,min= cor64_low,max = cor64_high),
                           b4=runif(numIterations,min= cor65_low,max = cor65_high),
                           cprime=runif(numIterations,min= cor61_low,max = cor61_high), #affect of x on y
                           rm1m2=runif(numIterations,min= cor32_low,max = cor32_high), #affect of mediator var 1 and mv2)
                           rm1m3=runif(numIterations,min= cor42_low,max = cor42_high), #affect of mediator var 3 on mv1
                           rm2m3=runif(numIterations,min= cor43_low,max = cor43_high),
                           rm1m4=runif(numIterations,min= cor52_low,max = cor52_high),
                           rm2m4=runif(numIterations,min= cor53_low,max = cor53_high),
                           rm3m4=runif(numIterations,min= cor54_low,max = cor54_high))
...
power_estimates_4mediators <- data.frame(a1b1 = rep(0,numIterations), a2b2 = rep(0,numIterations),
                                         a3b3 = rep(0,numIterations),a4b4 = rep(0,numIterations),
                                         difference12 =rep(0,numIterations), difference13 =rep(0,numIterations),
                                         difference14 =rep(0,numIterations),difference23 =rep(0,numIterations),
                                         difference24 =rep(0,numIterations),difference34 =rep(0,numIterations))
...
for(i in 1:numIterations){
  ...
  corr <- input_data[i,]
  if(input_method=="sc"){
  power_estimates_4mediators[i,] <- calc_4mediator_power(powReps=powReps, mcmcReps = mcmcReps, seed = seed,
...
power_means <- data.frame(a1b1_mean = mean(power_estimates_4mediators[,1]),
                          a2b2_mean = mean(power_estimates_4mediators[,2]),
                          a3b3_mean = mean(power_estimates_4mediators[,3]),
                          a4b4_mean = mean(power_estimates_4mediators[,4]),
                          difference12 = mean(power_estimates_4mediators[,5]),
                          difference13 = mean(power_estimates_4mediators[,6]),
                          difference14 = mean(power_estimates_4mediators[,7]),
                          difference23 = mean(power_estimates_4mediators[,8]),
                          difference24 = mean(power_estimates_4mediators[,9]),
                          difference34 = mean(power_estimates_4mediators[,10]))
